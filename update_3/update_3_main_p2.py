...
import update_3_utils_p2 as u3u_p2 # for custom "update_3_utils" utility functions
...
target_df, targets = u3u_p2.encode_target(df, "SECU_FLAG") # encoding dataframe with "SECU_FLAG" as classification target
targets_dict = u3u_p2.col_count_perc(target_df, "SECU_FLAG", len(df.index), 2) # count unique targets and calculate percentiles
...
datasets = u3u_p2.partition_df_repo_type_groups(target_df, single_type_groups) # [chemistry_df, astronomy_df, biology_df, others_df]

# Loop through all 4 of the partitioned dataset groups
for i in range(N): # N = 4
	dataset_dict = u3u_p2.col_count_perc(datasets[i], "SECU_FLAG", len(datasets[i].index), 2) # calculate percentitles for partitioned dataframes
...
for g in range(len(datasets)):
	...
	u3u_p2.print_parent_divider("effect", g, all_groups) # formatting printed output
	...
	for f in range(len(full_model_features)):
		...
		u3u_p2.compare_lists(group_feat_n_list, group_feat_i_list, u3u_p2.CD_LRG_DIFF)

#---------------------------------- Train/Build Full Model for Each Original Dataset ---------------------------------#

u3u_p2.print_parent_divider("fit") # formatting printed output
...
for i in range(N): # N = 4
	...
	u3u_p2.print_parent_divider("prior", i, all_groups) # formatting printed output
	...
	for j in range(N): # N = 4
		...
		for k in range(M): # M = 5
			u3u_p2.print_child_divider(model_classifiers[k]) # formatting printed output
		  ...
		  # Skipping datasets for testing purposes
			...
			host, guest = u3u_p2.set_host_and_guest(i, j, single_type_groups) # set proper host and guest
...
for i in range(N): # N = 4
	...
	u3u_p2.print_parent_divider("dnn", i, all_groups) # formatting printed output
	...
	for j in range(N): # N = 4
		...
		host, guest = u3u_p2.set_host_and_guest(i, j, single_type_groups)
		...
		acc, prec, rec, fscore = u3u_p2.get_dnn_pred_metrics(ground_truth, pred_mat)
		true_arr, pred_arr = u3u_p2.process_dfcol_and_mat(ground_truth, pred_mat)
...
for d in range(len(datasets)):
	resampled_datasets[d] = u3u_p2.resample_dataset(datasets[d], full_model_features, all_groups[d])

# Loop through all 4 of the resampled, partitioned dataset groups
for i in range(N): # N = 4
	resampled_dataset_dict = u3u_p2.col_count_perc(resampled_datasets[i], "SECU_FLAG", len(resampled_datasets[i].index), 2) # calculate percentitles for resampled dataframes
